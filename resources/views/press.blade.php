            ...
            <figure class="figure">
                <div class="d-flex justify-content-around">
                    @if ( !empty ( $event->filePhoto1) ) 
                        <img src="{{asset('storage/' .$event->filePhoto1 ) }}"  class="figure-img img-fluid rounded"  width="50%;" height="35%;">
                    @else
                        @if ( !empty ( $event->filePhoto2) ) 
                            <img src="{{asset('storage/' .$event->filePhoto2 ) }}"  class="figure-img img-fluid rounded"  width="50%;" height="35%;">
                        @else
                            @if ( !empty ( $event->filePhoto3) ) 
                                <img src="{{asset('storage/' .$event->filePhoto3 ) }}"  class="figure-img img-fluid rounded"  width="50%;" height="35%;">

                            @else
                                @if ( !empty ( $event->filePhoto4) ) 
                                    <img src="{{asset('storage/' .$event->filePhoto4 ) }}"  class="figure-img img-fluid rounded"  width="50%;" height="35%;">

                                @else
                                    @if ( !empty ( $event->filePhoto5) ) 
                                        <img src="{{asset('storage/' .$event->filePhoto5 ) }}"  class="figure-img img-fluid rounded"  width="50%;" height="35%;">

                                    @else
                                        @if ( !empty ( $event->filePhoto6) ) 
                                            <img src="{{asset('storage/' .$event->filePhoto6 ) }}"  class="figure-img img-fluid rounded"  width="50%;" height="35%;">
                ...
                <figcaption class="figure-caption text-right">
                    @if ( !empty ( $event->Titre) ) 
                        <h5 class="card-title text-center" ><strong style="color:#18d26e;font-weight:bold;text-align:center;font-size:30px;">{{$event->Titre}}</strong></h5>
                        @else
                        <p></p>
                    @endif
                    <div style="padding-right:200px;">
                        <a href="{{asset('events/'. $event->id) }}"class="btn btn-primary" style="font-family: 'Droid Arabic Kufi', Tahoma, Arial, Times New Roman, serif;font-size:20px;background-color:#18d26e;border-color:#18d26e;color:#fff;float:right;">إقرأ المزيد</a>
                    </div>
                </figcaption>
            </figure>
        </div>
